...
app.get("/navigate", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Navigation failed", details: err.message });
  }
});
...
app.get("/screenshot", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Screenshot failed", details: err.message });
  }
});
...
app.get("/", (req, res) => {
  res
    .type("text")
    .send(
      "Playwright MCP-style server. Use /navigate?url=... or /screenshot?url=..."
    );
});
